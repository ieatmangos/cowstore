...
const getExcited = (evt) => {
  ...
  // move ears
  const ear1 = document.getElementById("ear-left");
  const ear2 = document.getElementById("ear-right");
  // const x = -(wrapper.offsetWidth / 2 - evt.pageX) / 160;
  // const y = -(wrapper.offsetHeight / 2 - evt.pageY) / 160;
  //   console.log(eye1, eye2, x, y);
  if (ear1) {
    ear1.style.transform = `translateX(4px) rotate(45deg)`;
  }
  if (ear2) {
    ear2.style.transform = `translateX(-4px) rotate(-45deg)`;
  }
...
const reset = (evt) => {
  ...
  const ear2 = document.getElementById("ear-right");
  if (ear1) {
    ear1.style.transform = `translateX(0px) rotate(25deg)`;
  }
  if (ear2) {
    ear2.style.transform = `translateX(0px) rotate(-25deg)`;
  }
...
export default function Cow({ hov }) {
  useEffect(() => {
