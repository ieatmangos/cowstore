...
export default function Recover() {
  ...
  const handleClick = async (e) => {
    e.preventDefault();
    setHasError(false);
    const [act, error] = await recover(newAccount);
    if (act && act.success) {
      toast.success(`Email has been sent to ${newAccount.email}`);
      setCloseDown(true);
    }
    if (error) {
      setHasError(true);
      ...
      <div className="">
        <div className="max-w-lg mx-auto">
          ...
          {!closeDown ? (
              ...
              <div className="px-4 py-8 bg-white shadow sm:rounded-lg sm:px-10">
                ...
                {hasError && (
                  <p className={`pt-6 text-rose-600`}>
                    It looks like you may have entered the wrong email address
                    or password. Please double-check and try again. If
                    you&apos;re having trouble, feel free to contact us for
                    help.
                  </p>
                )}
              </div>
            ...
            <div className="px-4 py-8 mt-8 text-center bg-gray-50 sm:rounded-lg sm:px-10">
              <div className="block mb-3 text-sm font-medium text-gray-700">
